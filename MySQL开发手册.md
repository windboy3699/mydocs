...
这种方法只适用于不需要显示页码的场景，因为无法准确预测后几页的开始ID，所以缺点是不能随意查询某一页。
...
按参数名升序排序得到字符串"area=val2&sort=val3&type=val1"，然后md5字符串取前8位加上前缀得到缓存key，把该条件的查询结果总数做缓存。如果后面有相同条件的查询，直接从缓存读取。  
缓存内容为：params:"area=val2&sort=val3&type=val1",count:2000
